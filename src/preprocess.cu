    ...
    AffineMatrix2 affine_matrix)
{
    ...
    if (src_y1 < 0 || src_x1 < 0 || src_y1 >= trans.src_h || src_x1 >= trans.src_w) return;
    ...
    float scale)
{
    ...
    if (src_y1 < 0 || src_x1 < 0 || src_y2 >= srcH || src_x2 >= srcW) return;
...
void cuda_preprocess(cv::Mat &h_src, float* d_dst, int dst_h, int dst_w, cudaStream_t stream) {
    ...
    // input image hw
    int src_h = h_src.rows;
    int src_w = h_src.cols;
    int channel = 3;

    int src_size  = src_h * src_w * channel * sizeof(uint8_t);
    ...
    // resize_bilinear_gpu(d_dst, d_src, dst_h, dst_w, src_w, src_h, stream);
    dim3 dimBlock(32, 32);
    dim3 dimGrid(CEIL(dst_w, 32), CEIL(dst_h, 32));
   
    if (true) {
        warpaffine_init(src_h, src_w, dst_h, dst_w);
        warpaffine_BGR2RGB_kernel<<<dimGrid, dimBlock, 0, stream>>>(d_dst, d_src, trans, affine_matrix);  
        // save preprocess
        if (false)   saveCudaImage(d_dst, dst_w, dst_h, "../results/preprocess_version1_affine.jpg", stream); 
        ...
        bilinear_BGR2RGB_nhwc2nchw_shift_kernel<<<dimGrid, dimBlock, 0, stream>>>(d_dst, d_src, dst_w, dst_h, src_w, src_h, scale);   
        if (false)   saveCudaImage(d_dst, dst_w, dst_h, "../results/preprocess_version1_.jpg", stream);  
    }
...
void cuda_preprocess(uint8_t *src, int src_width, int src_height, float *dst, int dst_width, int dst_height, cudaStream_t stream)
{
    ...
    if (false)   saveCudaImage(dst, dst_width, dst_height, "../results/preprocess_version2.jpg", stream);
}
